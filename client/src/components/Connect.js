...
const Connect = (
  ...
  connection.on("IsLogined", (user) => {
    !user ? console.log("Cant login to that user!") : setCurrentUser(user);
  });

  connection.on("IsRegistered", (flag) => {
    flag ? console.log(`Registered!`) : console.log(`alredy in Database`);
  });

  connection.on("GetUsers", (users) => {
    !users ? console.log("Cant find users!") : setUsers(users);
  });
  ...
  connection.on("SendMessage", (flag) => {
    !flag ? console.log("Cant send message!") : console.log("Message sent!");
  });
